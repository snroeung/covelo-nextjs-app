...
## General
Talk like a caveman - no emotion, greetings, formalities,
Be straight forward and quick to the point
...
**Before running the checks below, autofix lint issues in unstaged changes:** run `npx eslint --fix $(git diff --name-only --diff-filter=ACMR -- '*.ts' '*.tsx')` to apply autofixes only to files with unstaged changes, then re-check `git diff` to confirm the fixes are sane before staging. Never run `npm run lint -- --fix` (or `--fix` with no path scope) across the whole repo — it will also rewrite generated/untracked directories like `coverage/` and `playwright-report/` that must never be edited or committed.
...
- **Desktop only** — Mobile Chrome is excluded for now; do not add mobile projects to `playwright.config.ts`
- axe-core accessibility assertions must pass in every E2E test (`results.violations` must be empty)
- Auth: `PLAYWRIGHT_ADMIN_EMAIL` and `PLAYWRIGHT_ADMIN_PASSWORD` must be set in `.env` or `.env.local`; run `npx playwright install` once to download browsers
- Auth is handled by `e2e/global-setup.ts` (runs before all tests, including in `--ui` mode) — do not add a `setup` project to `playwright.config.ts`

**Folder structure** — one folder per app route:
```
e2e/
  global-setup.ts       ← auth (runs before all tests)
  utils/
    admin-helpers.ts    ← shared helpers (createTransferBonus, etc.)
  home/
    home.spec.ts
  offers/
    offers-page.spec.ts ← general page tests (load, a11y, filters)
    offers-admin.spec.ts ← admin CRUD flows (create, edit, deactivate)
  flights/              ← add when flights E2E tests are written
  hotels/               ← add when hotels E2E tests are written
```
...
npm run test:e2e -- e2e/offers/              # one folder
npm run test:e2e -- --grep "offers"          # by name pattern
```
...
- Auth callback handler: `app/auth/callback/route.ts` — `app/auth/callback` is a directory, not a file
- `app/onboarding` is also a directory (contains `page.tsx`) — do not `Read` the directory path directly, it raises `EISDIR`
- tRPC client: `lib/trpc-client.ts`; tRPC API route: `app/api/trpc/[trpc]/route.ts`; server routers: `server/routers/` (`_app.ts`, `stays.ts`, `flights.ts`, `places.ts`, `offers.ts`)
- Offers admin components live under `components/offers/admin/` (not `components/offers/`): `AdminOfferEditor.tsx`, `AdminAdEditor.tsx`, `AdminAdsTable.tsx`, `AdminOffersTable.tsx`, `OffersAdminShell.tsx`
- Supabase clients: `lib/supabase/client.ts`, `lib/supabase/server.ts`, `lib/supabase/admin.ts`; migrations at `supabase/migrations/`
- Card images: `assets/cards/*.{avif,png}` only — there are NO `.webp` files. Always import card images via the `CARD_IMAGES` map in `lib/cardImages.ts`, never hardcode paths like `@/assets/cards/bilt_blue.webp` — `app/onboarding/page.tsx` has repeatedly broken the production build this way (recurring "Module not found" errors across multiple sessions)
...
- **No `src/` directory exists.** All source lives at root-level: `app/`, `components/`, `lib/`, `server/`, `contexts/`, `hooks/`, `supabase/`, `assets/`, `__tests__/`, `e2e/`. This trips up nearly every fresh Explore/general-purpose subagent whose prompt references `src/app`, `src/components`, or `src/lib` — when writing subagent prompts for this repo, explicitly state paths are at repo root, not under `src/`.
- **Use `proxy.ts` (not `middleware.ts`)** for route guards — `middleware.ts` is deprecated in this Next.js version; the project uses `proxy.ts` at the repo root
...
- Turbopack emits `AVIF image not supported` for every `.avif` in `assets/cards/` — build still succeeds; do not attempt to fix unless explicitly asked
- Vitest warns about `vite-tsconfig-paths` plugin detected — tests still pass; the plugin was intentionally removed from `vitest.config.ts`

<!-- headroom:learn:end -->

## Module Dependency Map

`DEPS.md` at repo root holds the reverse import map (`target << importers`) for all source files. It is regenerated automatically at session start by a SessionStart hook running `node scripts/dep-graph.mjs` — always fresh, never run the script manually. **Before editing any shared file (`lib/`, `contexts/`, `hooks/`, or any component), grep `DEPS.md` for the file path to get its exact importer list (blast radius) — do not read the whole file and do not grep the repo for imports.**
