...
export function CardSelector() {
  ...
  const countColor    = isDark ? 'text-cv-blue-400'  : 'text-gray-500';
  const issuerColor   = isDark ? 'text-cv-blue-400'  : 'text-gray-400';
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-md border text-sm font-medium transition-colors cursor-pointer text-left
                      ${selected
                        ? isDark
                          ? 'border-cv-blue-600 bg-cv-blue-900 text-cv-blue-300'
                          : 'border-cv-blue-600 bg-cv-blue-50 text-cv-blue-700'
                        : isDark
                          ? 'border-cv-blue-800 text-cv-blue-300 hover:border-cv-blue-600'
                          : 'border-gray-200 text-gray-700 hover:border-gray-400'
                      }`}
                  >
                    <span>{CARD_NAMES[cardId]}</span>
                    {selected && (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={3} className="text-cv-blue-600 shrink-0">
